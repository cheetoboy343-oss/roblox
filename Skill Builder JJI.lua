...
local Colors = {
    ...
    ButtonHover = Color3.fromRGB(0, 220, 255),
}
...
MinimizeButton.MouseButton1Click:Connect(function()
    ...
    local targetSize = IsMinimized and UDim2.new(0, 520, 0, 40) or UDim2.new(0, 520, 0, 480)
...
-- Keybind listener for remote firing
local KeyCodeMap = {
    Z = Enum.KeyCode.Z,
    X = Enum.KeyCode.X,
    C = Enum.KeyCode.C,
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    for key, keyCode in pairs(KeyCodeMap) do
        if input.KeyCode == keyCode then
            -- Check if holding Innates tool and has innate binding
            if IsHoldingInnatesTool() and RemoteInnateBindings[key] then
                FireSkillRemote(RemoteInnateBindings[key])
                return
            end
            
            -- Check if holding Skills tool and has skill binding
            if IsHoldingSkillsTool() and RemoteSkillBindings[key] then
